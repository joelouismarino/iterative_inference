...
def plot_metrics(train_val):
    def plotting_train_val(model, train_config, data, epoch, handle_dict, mode='val', optimizers=None, shuffle_data=True):
        output = train_val(model, train_config, data, mode=mode, optimizers=optimizers, shuffle_data=shuffle_data)
        if mode == 'train':
