...
class Plotter(object):
    """
    A plotting class for plotting with Visdom.
    ...
    def _init_plots(self):
        ...
        nans.fill(np.nan)
        n_inf_it = self.train_config['n_inf_iterations'] + 1
        ########################################################################
        ...
        ########################################################################
        # Average ELBO, conditional log likelihood, KL divergence
        ########################################################################
        handle_dict['elbo'] = plot_line(self.vis, nans, np.ones((1, 2)), legend=['Train', 'Val'],
                                      title='ELBO', xlabel='Epochs',
                                      ylabel='- ELBO (Nats)', xformat='log', yformat='log')
        handle_dict['cll'] = plot_line(self.vis, nans, np.ones((1, 2)), legend=['Train', 'Val'],
                                       title='Conditional Log Likelihood', xlabel='Epochs',
        ...
        #                               title='KL Divergence', xlabel='Epochs',
        #                               ylabel='KL Divergence (Nats)', xformat='log', yformat='log')
        ########################################################################
        # ELBO, conditional log likelihood, KL divergence over inference iterations
        ########################################################################
        it_legend = []
        for split in ['Train', 'Val']:
            for it_num in range(n_inf_it):
                it_legend.append(split + ', It. ' + str(it_num))
        handle_dict['elbo_it'] = plot_line(self.vis,
                                           nans.repeat(n_inf_it, 1),
                                           np.ones((1, 2 * n_inf_it)),
                                           legend=it_legend,
                                           title='ELBO Over Inf. Iterations',
                                           xlabel='Epochs',
                                           ylabel='- ELBO (Nats)',
                                           xformat='log', yformat='log')
        handle_dict['cll_it'] = plot_line(self.vis,
                                          nans.repeat(n_inf_it, 1),
                                          np.ones((1, 2 * n_inf_it)),
                                          legend=it_legend,
                                          title='Conditional Log Likelihood Over Inf. Iterations',
                                          xlabel='Epochs',
                                          ylabel='- Conditional Log Likelihood (Nats)',
                                          xformat='log', yformat='log')
        # handle_dict['kl_it'] = plot_line(self.vis,
        #                                  nans.repeat(n_inf_it, 1),
        #                                  np.ones((1, 2 * n_inf_it)),
        #                                  legend=it_legend,
        #                                  title='KL Divergence Over Inf. Iterations',
        #                                  xlabel='Epochs',
        #                                  ylabel='KL Divergence (Nats)',
        #                                  xformat='log', yformat='log')
        ########################################################################
        # Approximate posterior parameter magnitudes
        ########################################################################
        # it_legend = []
        # for split in ['Train', 'Val']:
        ...
        #                                          xformat='log', yformat='log')
        ########################################################################
        # Approximate posterior gradient magnitudes
        ########################################################################
        # it_legend = []
        # for split in ['Train', 'Val']:
        #     for it_num in range(self.train_config['inference_iterations']+1):
        ...
        it_legend = []
        for split in ['Train', 'Val']:
            for it_num in range(1, n_inf_it):
                it_legend.append(split + ', Iteration ' + str(it_num))
        handle_dict['inf_improvement'] = plot_line(self.vis,
                                                   nans.repeat(n_inf_it - 1, 1),
                                                   np.ones((1, 2*(n_inf_it-1))),
                                                   legend=it_legend,
    ...
    def plot(self, out_dict, train_val):
        ...
        metrics = [out_dict['elbo'], out_dict['cll'], out_dict['kl']]
        ...
        self._plot_inf_improvement(out_dict['elbo'], train_val)
    ...
    def _plot_metrics(self, metrics, train_val):
        """
        Plot the average total and per iteration metrics.
        ...
        # average metrics
        update_trace(self.vis, np.array([-elbo[:, -1].mean()]),
                     np.array([self.epoch]).astype(int),
                     win=self.handle_dict['elbo'], name=train_val)
        update_trace(self.vis, np.array([-cll[:, -1].mean()]),
                     np.array([self.epoch]).astype(int),
                     win=self.handle_dict['cll'], name=train_val)
        # update_trace(self.vis, np.array([kl[:, -1].mean()]),
        #              np.array([self.epoch]).astype(int),
    ...
    def _plot_inf_improvement(self, elbo, train_val):
        """
        Plot inference improvement as a percentage of initial estimate.

        Args:
            elbo (ndarray): shape is [n_examples x n_inf_iter]
            train_val (str): either 'Train' or 'Val', determines plotting behavior
        """
        for it_num in range(1, self.train_config['n_inf_iterations']+1):
            improvement = 100. * ((elbo[:, 0] - elbo[:, it_num]) / elbo[:, 0])
